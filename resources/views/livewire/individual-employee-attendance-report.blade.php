<div class="container-fluid pb-5">
                ...
                <div class="row align-items-center">
                    <div class="col-md-4">
                        ...
                        <div class="text-muted small">
                            ...
                            {{ $selected_employee->department->name ?? '—' }} {{ $selected_employee->designation ? '/ ' . $selected_employee->designation->name : '' }} &middot;
                            {{ $selected_employee->campus->name ?? '—' }}
                        </div>
                    </div>
                    <div class="col-md-8">
                    ...
                    <tbody>
                        @forelse($records as $rec)
                            <tr>
                                ...
                                <td class="text-muted">{{ $rec->time_in ? \Carbon\Carbon::parse($rec->time_in)->format('h:i A') : '—' }}</td>
                                <td class="text-muted">{{ $rec->time_out ? \Carbon\Carbon::parse($rec->time_out)->format('h:i A') : '—' }}</td>
                                <td class="text-muted">{{ $rec->remarks ?: '—' }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="text-center py-5 text-muted">No attendance records for this date range.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    @else
        <div class="text-center py-5 text-muted">
            ...
            Search and select an employee above to view their attendance report.
        </div>
    @endif
</div>
