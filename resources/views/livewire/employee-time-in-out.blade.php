<div class="container-fluid pb-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 p-3 bg-white rounded-3 shadow-sm">
        <div>
            <h5 class="mb-0 fw-bold text-dark"><i class="fas fa-hand-pointer me-2 text-primary"></i> Employee Time In/Out</h5>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 small mt-1">
                    <li class="breadcrumb-item"><a href="/dashboard" class="text-decoration-none">Home</a></li>
                    <li class="breadcrumb-item active">Employee Time In/Out</li>
                </ol>
            </nav>
        </div>
    </div>
                ...
                <table class="table table-hover table-bordered align-middle mb-0" style="font-size: 0.82rem;">
                    ...
                    <tbody>
                        @foreach($roster as $employeeId => $entry)
                            <tr class="{{ $entry['locked'] ? 'table-secondary' : '' }}">
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm w-100" role="group">
                                        <button type="button" @disabled($entry['locked']) wire:click="$set('roster.{{ $employeeId }}.status', 'present')" class="btn {{ $entry['status'] === 'present' ? 'btn-success' : 'btn-outline-success' }}">Present</button>
                                        <button type="button" @disabled($entry['locked']) wire:click="$set('roster.{{ $employeeId }}.status', 'absent')" class="btn {{ $entry['status'] === 'absent' ? 'btn-danger' : 'btn-outline-danger' }}">Absent</button>
                                        <button type="button" @disabled($entry['locked']) wire:click="$set('roster.{{ $employeeId }}.status', 'leave')" class="btn {{ $entry['status'] === 'leave' ? 'btn-warning' : 'btn-outline-warning' }}">Leave</button>
                                        <button type="button" @disabled($entry['locked']) wire:click="$set('roster.{{ $employeeId }}.status', 'late')" class="btn {{ $entry['status'] === 'late' ? 'btn-info' : 'btn-outline-info' }}">Late</button>
                                    </div>
                                </td>
                                <td><input type="time" @disabled($entry['locked']) wire:model="roster.{{ $employeeId }}.time_in" class="form-control form-control-sm border-0 bg-light"></td>
                                <td><input type="time" @disabled($entry['locked']) wire:model="roster.{{ $employeeId }}.time_out" class="form-control form-control-sm border-0 bg-light"></td>
                                <td><input type="text" @disabled($entry['locked']) wire:model="roster.{{ $employeeId }}.remarks" class="form-control form-control-sm border-0 bg-light" placeholder="Optional..."></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
