<div class="container-fluid pb-5">
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header py-3 d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #1e293b, #334155);">
            ...
            <span class="badge bg-white text-dark px-3 py-2">{{ count($rows) }} employee(s)</span>
        </div>
        <div class="table-responsive">
            <table class="table table-hover table-bordered align-middle mb-0" style="font-size: 0.82rem;">
                ...
                <tbody>
                    @forelse($rows as $row)
                        <tr>
                            <td class="ps-3 fw-bold">{{ $row['employee']->first_name }} {{ $row['employee']->last_name }}</td>
                            <td class="text-muted">{{ $row['employee']->emp_no }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-5">
                                ...
                                <p class="text-muted mb-0">No employees or attendance records found for the selected filters.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
