<div class="container-fluid min-vh-100">
    <div class="row g-3 mb-4 card shadow-sm border-0 rounded-4 overflow-hidden mx-0 bg-white shadow-none border-bottom">
        <div class="col-md-9 p-4">
            <h4 class="fw-bold mb-1"><i class="fas fa-layer-group text-primary me-2"></i> Installment-Wise Billing Engine</h4>
            <p class="text-muted small mb-0">Split a student's monthly bill into multiple challans due on staggered dates.</p>
        </div>
    ...
    <div class="row g-4">
        <div class="col-xl-4 col-lg-5">
            <div class="card shadow-sm border-0 rounded-4 p-4 sticky-top shadow-lg" style="top: 130px; z-index: 10;">
                ...
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        <label class="form-label small fw-bold">No. of Installments</label>
                        <input type="number" min="2" max="12" wire:model="installments" class="form-control bg-light border-0 shadow-sm">
                        @error('installments') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-6">
                        <label class="form-label small fw-bold">Days Between</label>
                        <input type="number" min="1" max="90" wire:model="interval_days" class="form-control bg-light border-0 shadow-sm">
                        @error('interval_days') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                    </div>
                </div>

                <div class="mb-4">
                    <label class="form-label small fw-bold">First Installment Due Date <span class="text-danger">*</span></label>
                ...
                @if (session()->has('message'))
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            Swal.fire({ icon: 'success', title: 'Success!', text: "{{ session('message') }}", confirmButtonColor: '#0d6efd' });
                        });
                    </script>
                @endif
            </div>
        ...
        <script>
            $wire.on('challans-generated', (data) => {
                Swal.fire({ icon: 'success', title: 'Generation Complete!', text: data.message, confirmButtonColor: '#0d6efd' });
            });
            $wire.on('challans-error', (data) => {
                Swal.fire({ icon: 'error', title: 'Oops...', text: data.message, confirmButtonColor: '#dc3545' });
            });
        </script>
        @endscript
        ...
        <div class="col-xl-8 col-lg-7">
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light sticky-top">
                            <tr class="tiny fw-bold text-uppercase text-muted border-bottom">
                                <th class="ps-4" width="40px">
                                    <div class="form-check"><input type="checkbox" wire:model.live="selectAll" class="form-check-input"></div>
                                </th>
                                <th>Reg No</th>
                                <th>Student Name</th>
                                <th>Campus / Class</th>
                        ...
                        <tbody>
                            @forelse($this->filteredStudents as $student)
                                <tr>
                                    <td class="ps-4">
                                        <div class="form-check"><input type="checkbox" wire:model.live="selected_students" value="{{ $student->id }}" class="form-check-input"></div>
                                    </td>
                                    ...
                                    <td class="text-end pe-4">
                                        ...
                                        @else
                                            <span class="badge bg-danger bg-opacity-10 text-danger small border border-danger border-opacity-25 px-3">Missing Plan</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
            ...
            <div class="mt-4 card border-0 shadow-sm rounded-4 bg-primary bg-opacity-10">
                <div class="card-body p-4 d-flex align-items-center">
                    <div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center me-4 shadow" style="width: 50px; height: 50px;">
                        <i class="fas fa-shield-alt"></i>
                    </div>
                    <div>
                        <h6 class="fw-bold mb-1">Split Billing, Full Protection</h6>
                        <p class="small mb-0 text-muted">Each installment's amount is a proportional share of every fee particular. The engine still skips students who already have a bill (regular or installment) for <strong>{{ strtoupper($month) }} {{ $year }}</strong>.</p>
                    </div>
                </div>
            </div>
        </div>
