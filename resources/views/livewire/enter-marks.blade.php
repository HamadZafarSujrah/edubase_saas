<div class="container-fluid pb-5">

    {{-- Page Header --}}
    <div class="d-flex justify-content-between align-items-center mb-4 p-3 bg-white rounded-3 shadow-sm">
        <div>
            <h5 class="mb-0 fw-bold text-dark"><i class="fas fa-desktop me-2 text-primary"></i> Add / Update / View Marks</h5>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 small mt-1">
                    <li class="breadcrumb-item"><a href="/dashboard" class="text-decoration-none">Home</a></li>
                    <li class="breadcrumb-item active">Marks Entry</li>
                </ol>
            </nav>
        </div>
    </div>

    @if (session()->has('message'))
        <div class="alert alert-success border-0 shadow-sm mb-4"><i class="fas fa-check-circle me-2"></i>{{ session('message') }}</div>
    @endif
    @if (session()->has('error'))
        <div class="alert alert-danger border-0 shadow-sm mb-4"><i class="fas fa-exclamation-circle me-2"></i>{{ session('error') }}</div>
    @endif
    @if($sms_result_message)
        <div class="alert alert-info border-0 shadow-sm mb-4"><i class="fas fa-sms me-2"></i>{{ $sms_result_message }}</div>
    @endif

    {{-- Filters --}}
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-3">
            <div class="row g-3 align-items-end">
                <div class="col-md-5">
                    <label class="small fw-bold text-muted mb-1">Exam</label>
                    <select wire:model.live="exam_id" class="form-select border-0 bg-light shadow-sm">
                        <option value="">Select exam...</option>
                        @foreach($exams as $e) <option value="{{ $e->id }}">{{ $e->name }} — {{ $e->schoolClass->name ?? '' }}</option> @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="small fw-bold text-muted mb-1">Section (Optional)</label>
                    <select wire:model="section_id" class="form-select border-0 bg-light shadow-sm" @if(!$exam_id) disabled @endif>
                        <option value="">All Sections</option>
                        @foreach($sections as $s) <option value="{{ $s->id }}">{{ $s->name }}</option> @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <button wire:click="loadRoster" class="btn btn-primary w-100 fw-bold rounded-3 shadow-sm">
                        <i class="fas fa-list me-2"></i> Load Roster
                    </button>
                </div>
            </div>
        </div>
    </div>

    @if($loaded)
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div class="card-header py-3 d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #1e293b, #334155);">
                <span class="text-white fw-bold"><i class="fas fa-users me-2"></i> Roster — {{ count($roster) }} student(s)</span>
                <div class="d-flex gap-2">
                    <button wire:click="sendResultSms" wire:confirm="Send result SMS to all students in this roster?" class="btn btn-sm btn-info rounded-pill px-3"><i class="fas fa-sms me-1"></i> Send Result SMS</button>
                </div>
            </div>
            <div class="table-responsive" style="max-height: 550px;">
                <table class="table table-hover table-bordered align-middle mb-0" style="font-size: 0.8rem;">
                    <thead>
                        <tr style="background:#1e293b; color:#fff; font-size:0.7rem;" class="text-uppercase">
                            <th class="ps-3">Student</th>
                            <th width="100">Admission No.</th>
                            @foreach($subjects as $cs)
                                <th class="text-center">{{ $cs->subject->name ?? '—' }}<br><span class="text-muted" style="font-weight:normal;">/ {{ $cs->marks_total }}</span></th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($roster as $studentId => $entry)
                            <tr>
                                <td class="ps-3 fw-bold">{{ $entry['name'] }}</td>
                                <td class="text-muted">{{ $entry['admission_no'] }}</td>
                                @foreach($subjects as $cs)
                                    <td class="text-center">
                                        <input type="number" step="0.01" min="0" max="{{ $cs->marks_total }}"
                                               wire:model="roster.{{ $studentId }}.marks.{{ $cs->subject_id }}.obtained"
                                               class="form-control form-control-sm border-0 bg-light text-center" style="width: 80px; margin: 0 auto;">
                                    </td>
                                @endforeach
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
            <div class="card-footer bg-white border-0 py-3">
                <button wire:click="save" wire:loading.attr="disabled" class="btn btn-primary fw-bold px-5 py-2 rounded-3 shadow-sm">
                    <i class="fas fa-save me-2"></i> Save Marks
                </button>
            </div>
        </div>
    @endif
</div>
